...
import com.eventmanager.eventmanager.dto.AuthResponseDTO;
import com.eventmanager.eventmanager.dto.ResetPasswordRequest;
import com.eventmanager.eventmanager.dto.SigninRequestDTO;
import com.eventmanager.eventmanager.dto.SignupRequestDTO;
import com.eventmanager.eventmanager.model.ResetToken;
import com.eventmanager.eventmanager.model.User;
import com.eventmanager.eventmanager.model.VerificationToken;
import com.eventmanager.eventmanager.model.enmus.Role;
import com.eventmanager.eventmanager.repository.ResetTokenRepository;
import com.eventmanager.eventmanager.repository.UserRepository;
import com.eventmanager.eventmanager.repository.VerificationTokenRepository;
import com.eventmanager.eventmanager.services.EmailService;
...
public class AuthController {
    ...
    public AuthResponseDTO signup(@Valid @RequestBody SignupRequestDTO signupRequest) throws MessagingException, IOException {
        ...
        if (userRepository.findByEmail(signupRequest.getEmail()).isPresent()) {
            return new AuthResponseDTO("Email is already in use", null);
        }
        ...
        // Send email
        emailService.sendOtpEmailHtml(user.getEmail(),user.getName(), otp);
        return new AuthResponseDTO("User registered. Please verify OTP sent to your email.", null);
    }
    ...
    public ResponseEntity<?> signin(@Valid @RequestBody SigninRequestDTO signinRequest) {
        ...
        return ResponseEntity.ok(new AuthResponseDTO("Sign-in successful", token));
    }
    ...
    public ResponseEntity<?> forgotPassword(@RequestParam String email) throws MessagingException, IOException {
        ...
        emailService.sendResetPasswordEmailHtml(user.get().getEmail(),user.get().getName(), resetLink);
    ...
    public AuthResponseDTO googleLoginSuccess(@RequestParam("token") String googleToken) {
        ...
        return new AuthResponseDTO("Google login successful", googleToken);
    }
}
